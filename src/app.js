import { changeIcons, changeBgImage } from './setImgs';
...
async function setData(location) {
  try {
    const response = await fetch(`http://api.openweathermap.org/data/2.5/weather?q=${location}&APPID=02d0c041d3aadabf5500232268440a40`);
    const getData = await response.json();
    const currentTemperature = Math.round(getData.main.temp - 273.15);
    const weatherDescription = getData.weather[0].description;
    const city = getData.name;
    const weatherSetImg = getData.weather[0].main.toLowerCase();
    const feelsLike = Math.round(getData.main.feels_like - 273.15);
    const { humidity } = getData.main;
    const rainChance = getData.main.humidity;
    const windSpeed = getData.wind.speed;
